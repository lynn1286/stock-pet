...
export function useStockConfig() {
  ...
  // 删除撤销
  const deletedStockRef = useRef<StockConfig | null>(null);
  const undoTimerRef = useRef<ReturnType<typeof setTimeout>>(undefined);

  // 加载配置（含重试）
  const loadConfig = useCallback(async (retries = 2): Promise<void> => {
    try {
  ...
  // 删除股票（含撤销）
  const removeStock = useCallback(async (stock: StockConfig) => {
    try {
      deletedStockRef.current = { ...stock };
  ...
  const undoDelete = useCallback(async () => {
    const stock = deletedStockRef.current;
    if (!stock) return;
  ...
  // 更新股票（份额/成本价）
  const updateStock = useCallback(async (secid: string, quantity: number, costPrice: number) => {
    try {
  ...
  // 设置主股票
  const setPrimary = useCallback(async (secid: string) => {
    try {
